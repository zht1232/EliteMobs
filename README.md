...
| 💚 再生 `REGEN` | 再生 I~IV |
| 🛡 抗性 `RESIST` | 抗性 I~IV |
| 🔥 火焰 `FIRE` | 着火时获得抗火 N 秒（Lv.N = N 秒，触发后冷却 5 秒） |
...
### 🛒 强化菜单 / 商城（`/emmenu`）
内置多级箱子菜单（`/emmenu`，别名 `/emgem`/`/emrune`/`/emruneshop`），金币 / 点券二选一支付：
- **宝石商城**：10 种宝石按等级选购（Lv.1-10），价格越高级越贵（Lv.N = 基础价 × (1 + N(N-1)/10)，Lv.10 = 10 倍基础价），每日每人限购可配置
- **符文商城**：每日随机上架 3 种（符文 / 保护符，限量，次日刷新）
- **我的状态 / 使用说明**：余额查看与玩法说明
- **管理页**：发放物品、生成精英、清除附近、配置设置、**商城价格调整**（游戏内直接调价并即时保存 config.yml）
...
| `/em fixgear [玩家名]` | `elitemobs.admin` | 修复历史升合金的淬炼装备属性（默认全体在线玩家） |
| `/emmenu` | `elitemobs.use` | 打开强化菜单（宝石商城 / 符文商城 / 价格管理等） |
...
### v29.16.0
- 🎨 **菜单美化重做**：彩色玻璃板边框 + 白色磨砂内部（每菜单独立主题色），去除花哨方块填充
- 💰 **管理页新增商城价格调整**：`/emmenu → 管理 → 商城价格` 游戏内调整宝石/符文/消耗品价格并即时保存
- 📈 **宝石定价重构**：基础价 5000 金币 / 200 点券；越高级越贵（Lv.10 封顶 5 万金币）；符文/保护符/拆卸器点券全面下调
- 🔥 **修复抗火符文等级无效**：着火检测触发，抗火时长随等级提升（Lv.N = N 秒），触发后 5 秒冷却
- ⚒️ **修复下界合金升级属性不变**：锻造升级后自动修正武器攻击基准 / 护甲韧性 / 击退抗性，保留淬炼加成
- 🛠 **新增 `/em fixgear`**：一键修复历史升合金的淬炼装备属性（全体在线玩家 / 指定玩家，含末影箱）

### v29.13.3
- 🗂 **强化菜单系统**：多级箱子菜单（主界面/宝石商城/符文商城/三级购买页/状态/说明/管理），金币点券二选一
- ⚒️ **铁砧合成**：宝石+宝石、符文+符文同级合成高一级（Lv.10 满级不可再合）
- 🛒 **商城上架**：保护符/拆卸器；符文每日限量上架次日刷新
- ⚙️ **管理页**：发放宝石/符文/保护符/拆卸器、生成精英、清除附近、配置设置（即时保存）
