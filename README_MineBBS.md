...
### v29.21.0
- 🗄️ **精英/Boss 全量 SQLite 持久化（摆脱模拟距离限制）**：所有生成的精英与 Boss 数据写入 `plugins/EliteMobs/elitemobs.db`（内置 sqlite-jdbc 驱动，无需额外依赖/联网）。区块卸载时实体从世界移除、数据回写；玩家接近时按记录"物化"还原（含等级/职业/词缀/剩余血量/二阶段状态）。精英/Boss 从此固定存在于世界坐标上，不再依赖玩家附近自然生成，也不受模拟距离限制
- ☠️ **Boss 改为在世界远端直接布署（替代"玩家附近精英晋升"）**：每隔 `boss.spawn.interval-seconds`（默认 10 分钟）在随机玩家周围 150-800 格选安全点布署 Boss（数据入库 + 全服广播坐标，此时无实体）；玩家接近 `boss.spawn.materialize-distance`（默认 48 格）时 Boss 落雷登场。彻底解决"Boss 在未加载区块凭空消失/走过去不见了"——远距离时只有数据没有实体
- 💾 **重启保留**：停服时全量回写存活精英/Boss 血量与位置；重启后潜伏 Boss 位置会再次广播（`boss.spawn.announce-on-start`），玩家接近即重新现身，无需从头打
- ⚙️ **新增配置**：`persistence.*`（开关/入库间隔/精英物化距离）与 `boss.spawn.*`（布署间隔/并发上限/距离/等级范围/物化距离/启动广播）。`/em boss` 指令保留可用
