...
- ☠️ **Boss 改为在世界远端直接布署（替代"玩家附近精英晋升"）**：数据入库 + 全服广播坐标，此时无实体；玩家接近 `boss.spawn.materialize-distance`（默认 48 格）时 Boss 落雷登场。彻底解决"Boss 在未加载区块凭空消失/走过去不见了"——远距离时只有数据没有实体
- ⚖️ **布署频率/距离权重动态化（不再固定 10 分钟）**：
  - **频率**：`nextDelay = base-interval(默认 1200s) × 人数因子 × 昼夜因子 × Boss数因子 × 击杀活跃因子 × 抖动(0.7~1.3)`。10 人=人数因子 1.0（人多更快、人少更慢，封顶 ×0.5~×2.0）；夜间更快（×0.7）、白天更慢（×1.3）；Boss 多了间隔拉长防扎堆；最近 30 分钟击杀精英/Boss 越活跃间隔越短（每 10 击杀 -10%，下限 ×0.6）。实际间隔约 **9~60 分钟**（典型 15~40 分钟），`min-interval-seconds` 兜底防刷
...
- ⚙️ **新增配置**：`persistence.*`（开关/入库间隔/物化距离/过期时限）与 `boss.spawn.*`（动态间隔权重/距离/等级范围/群系权重/物化距离/过期时限）。`/em boss` 指令保留可用；`tryPromoteToBoss`（旧晋升）已移除
